...
public class third_main extends AppCompatActivity {
    ...
    String get_reason1,total_reason,get_reason2,get_reason3,get_reason4,get_reason5,get_reason6;
    String get_othertool1,get_othertool2,get_othertool3,get_tool1,get_tool2,total_othertools,total_tools;
                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                       if(task.isSuccessful()){
                           ...
                           if (doc.exists()) {
                               ...
                               get_reason1=doc.getString("reason1");
                               if(!"".equals(get_reason3)){
                                   get_reason1=get_reason1+"\n";
                               }
                               get_reason2=doc.getString("reason2");
                               if(!"".equals(get_reason2)){
                                   get_reason2=get_reason2+"\n";
                               }
                               get_reason3=doc.getString("reason3");
                               if(!"".equals(get_reason3)){
                                   get_reason3=get_reason3+"\n";
                               }
                               get_reason4=doc.getString("reason4");
                               if(!"".equals(get_reason4)){
                                   get_reason4=get_reason4+"\n";
                               }
                               get_reason5=doc.getString("reason5");
                               if(!"".equals(get_reason5)){
                                   get_reason5="治療因素:CVVH或IABP需約束單一側\n下肢肢體\n";
                               }

                               get_reason6=doc.getString("reason6");
                               if(!"".equals(get_reason6)){
                                   get_reason6=get_reason6+"\n";;
                               }
                               total_reason=get_reason1+get_reason2+get_reason3+get_reason4+get_reason5+get_reason6;
                               ...
                               total_tools=get_tool1+get_tool2;
